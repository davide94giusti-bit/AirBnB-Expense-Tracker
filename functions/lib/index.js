...
exports.deleteUserFromApartment = exports.createUserForApartment = void 0;
const functions = require("firebase-functions");
const admin = require("firebase-admin");
admin.initializeApp();
exports.createUserForApartment = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
...
exports.deleteUserFromApartment = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be logged in');
    }
    const { uid, apartmentId } = data;
    if (!uid || !apartmentId) {
        throw new functions.https.HttpsError('invalid-argument', 'Missing uid or apartmentId');
    }
    try {
